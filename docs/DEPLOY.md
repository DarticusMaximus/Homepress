...
Before you clone and compose up, have the following ready:
...
- **Podman** (primary / committed verified path), with Compose support (`podman compose`).
...
The `web` image bakes `NEXT_PUBLIC_APPWRITE_*` in at **build** time (compose `build.args`). After changing those values in `.env`, rebuild web (`podman compose build web` then `up -d`) so the client bundle picks them up. Server-side keys from `env_file` apply on container start/restart.
...
The same `compose.yaml` often works with `docker compose` (e.g. `docker compose up -d`). **Only Podman** (`podman compose`) is the committed, verified path for this repo.
